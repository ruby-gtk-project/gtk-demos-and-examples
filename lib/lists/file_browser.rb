...
class FileBrowserDemo
  ...
  def up_button = @up_button ||= Gtk::Button.new
  def view_switcher_box = @view_switcher_box ||= Gtk::Box.new(:horizontal, 0)
  def scrolled_window = @scrolled_window ||= Gtk::ScrolledWindow.new
  def grid_view = @grid_view ||= Gtk::GridView.new
  ...
  def paged_factory
    @paged_factory ||= Gtk::SignalListItemFactory.new.tap do |f|
      ...
      f.signal_connect('bind') do |_, item|
        ...
        box = item.child
        image = box.first_child
        vbox = image.next_sibling
        name_label = vbox.first_child
        size_label = name_label.next_sibling
        type_label = size_label.next_sibling

        image.gicon = info.icon if info
        name_label.label = info&.display_name || ''
        size_label.label = info ? GLib.format_size(info.size) : ''
        type_label.label = info&.content_type || ''
      end
    end
  end
  ...
  def navigate_up
    parent = dir_list.file&.parent
    dir_list.file = parent if parent
  end

  def on_file_activated(pos)
    info = file_selection.get_item(pos)
    return unless info

    if info.file_type == :directory
      file = info.attribute_object('standard::file')
